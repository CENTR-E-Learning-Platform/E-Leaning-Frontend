...
const UpcomingClassItem: React.FC<ClassItemProps> = ({ cls }) => {
  const startDate = cls.startTime ? new Date(cls.startTime) : null;
  const isValidDate = startDate !== null && startDate.toString() !== "Invalid Date";
  const formattedTime = startDate && isValidDate
    ? startDate.toLocaleTimeString([], { hour: "numeric", minute: "2-digit", hour12: true })
    : "--:--";
  const [time, period] = formattedTime.split(" ");

  const diffMinutes = startDate && isValidDate
    ? Math.round((startDate.getTime() - new Date().getTime()) / 60000)
    : null;
  const statusText = diffMinutes === null
    ? "No schedule"
    : diffMinutes < 0
      ? "Started"
      : diffMinutes < 60
  ...
  const statusColor = diffMinutes !== null && diffMinutes < 30 ? "#CC3363" : "#2DB584";
    ...
    <div className="flex flex-row justify-between w-full items-center h-[100px] font-['Poppins']">
            ...
            <div className="flex flex-row items-center">
              {shownAvatars.length > 0 ? (
                shownAvatars.map((avatar, index) => (
                  ...
                  >
                    <img src={avatar} alt={`attendee-${index}`} className="w-full h-full object-cover" />
                  </div>
                ))
              ) : (
      ...
      <div className="flex flex-row items-center gap-[24px]">
        <div className="flex flex-row items-center gap-[8px]">
          <div className="w-[8px] h-[8px] rounded-full" style={{ backgroundColor: statusColor }} />

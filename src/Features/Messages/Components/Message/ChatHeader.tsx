      ...
      <div className="flex flex-row items-center p-0 gap-[15.2px] flex-1 min-w-0 h-[36px] mr-[20px]">
          ...
          <div className="w-[38px] h-[36px] rounded-full overflow-hidden bg-gray-200">
            {isGroup ? (
              selectedConversation?.groupPicture ? (
                <img src={selectedConversation.groupPicture} alt="Avatar" className="w-full h-full object-cover" />
              ) : (
                <DefaultAvatar name={selectedConversation?.name || "Group"} className="w-full h-full text-[14px]" />
              )
            ) : (
              selectedConversation?.otherUserPicture ? (
                <img src={selectedConversation.otherUserPicture} alt="Avatar" className="w-full h-full object-cover" />
              ) : (
                <DefaultAvatar name={selectedConversation?.otherUserName || "User"} className="w-full h-full text-[14px]" />
              )
            )}
          </div>
        ...
        <div className="flex flex-col items-start p-0 h-[31.5px] flex-1 min-w-0">
          ...
          <div className="flex flex-col items-start p-0 self-stretch w-full h-[13.5px]">
            <span className={`block w-full font-semibold text-[9.5px] leading-[13.5px] tracking-[0.475px] uppercase truncate ${isGroup ? "text-[#525FE1]" : (selectedConversation?.isOnline ? "text-[#16A34A]" : "text-[#9CA3AF]")} `}>

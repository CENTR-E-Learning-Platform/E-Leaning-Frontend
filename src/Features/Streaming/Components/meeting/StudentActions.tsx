...
interface StudentActionsProps {
  ...
  profileImage?: string;
  width: number;
  muteToggleP?: (isMicOn: boolean) => void;
  cameraToggleP?: (isCamOn: boolean, allowMic: boolean) => void;
  shareToggleP?: (isShareOn: boolean, allowMic: boolean) => void;
  lowerHandP?: () => void;
  Partici: any;
  isRais?: string[];
}
...
const StudentActions = ({
  ...
  const isMicOn = remoteParticipant?.isMicrophoneEnabled ?? false;
  const isCamOn = remoteParticipant?.isCameraEnabled ?? false;
  const isShareOn = remoteParticipant?.isScreenShareEnabled ?? false;
  const allowMic = remoteParticipant?.permissions?.canPublish ?? false;
  const isHandRaised = isRais?.includes(Partici.name) ?? false;
  ...
  const toggleMenu = () => setCheckIdentity(!isOpen ? Partici.identity : "");
        ...
        <div className="flex items-center flex-1 min-w-0">
          {profileImage ? (
            <img
              ...
              className="w-[40px] h-[40px] rounded-full me-[13px] object-cover shrink-0"
              alt=""
            />
          ) : (
          ...
          {width < 1250 && (
            <div className="relative group flex items-center cursor-pointer min-w-0 pr-2">
              <h1 className="text-[16px] text-[#F9FBFC] truncate">{name}</h1>
          ...
          {isOpen && (
            <div className="absolute top-[120%] right-0 z-50">
              <ParticipantList
                ...
                onMuteToggle={() => muteToggleP?.(isMicOn)}
                onCameraToggle={() => cameraToggleP?.(isCamOn, allowMic)}
                onShareToggle={() => shareToggleP?.(isShareOn, allowMic)}
                onLowerHand={lowerHandP}
              />
            </div>
          )}
        </div>

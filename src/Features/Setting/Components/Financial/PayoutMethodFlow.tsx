...
export const PayoutMethodFlow = () => {
  const [selectedMethod, setSelectedMethod] = useState<'mobile' | 'card' | undefined>();
  const [phoneNumber, setPhoneNumber] = useState('');
  const [pin, setPin] = useState('');
  const [otp, setOtp] = useState('');

  return (
    /* 1. إضافة items-center لتوسيط المحتوى أفقياً داخل الكونتينر الرئيسي */
    <div className="flex flex-col justify-center gap-[30px] items-center bg-white border border-[#E8EAED] rounded-lg p-[30px] w-full max-w-[750px] mx-auto">
      ...
      {/* القسم العلوي: العنوان وأزرار الاختيار */}
      <div className="box-border flex flex-row justify-between items-start w-full max-w-[632px]">
        ...
        <button className="box-border flex flex-row justify-center items-center px-4 py-[16px] gap-2 w-[90px] h-[43px] border-2 border-[#525FE1] rounded-lg bg-transparent cursor-pointer hover:bg-[#525FE1] hover:bg-opacity-5 transition-colors">
        ...
        <div className="flex flex-col items-center p-0 gap-[16px] w-full max-w-[632px]">
          <div className="flex flex-col items-start p-0 gap-[8px] w-full">
            ...
            <input
              ...
              className="box-border flex flex-row items-center px-[16px] py-[12px] w-full h-[52px] bg-white border-2 border-[#D1D5DB] rounded-lg italic text-[18px] text-[#2A2D34] placeholder:text-[#6D7588] outline-none focus:border-[#525FE1] transition-colors"
            />
          </div>

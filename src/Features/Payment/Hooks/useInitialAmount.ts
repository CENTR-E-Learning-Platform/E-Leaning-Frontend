...
import { payByWallet } from "../Services/payByWallet";
export const useInitialAmount = () => {
    ...
    const sendPayByWallet = async (mobileNumber: string, amount: number) => {
        try {
            const response = await payByWallet(mobileNumber, amount);
            console.log(response.data);
    ...
    return { sendAmount, sendpaymob, sendingcharge, sendWallet, sendPayByWallet }
}
